...
| 🇮🇩 **Indonesian** | Casual / Laughing | `Wah kamu bisa juga! Aku nggak nyangka lho. <laugh> <laugh>` | [▶️ Play](https://github.com/user-attachments/files/28419491/3.wav) |
...
| 🇺🇸 **English** | Casual / Laughing | `I can't believe you did that it's so funny! <laugh> <laugh> <laugh>` | [▶️ Play](https://github.com/user-attachments/files/28419511/5.wav) |
...
| 🇪🇸 **Spanish** | Narrative / Breathing | `<breath> <breath> Hola, bienvenidos a nuestro canal de YouTube.` | [▶️ Play](https://github.com/user-attachments/files/28419523/7.wav) |
...
> ⚠️ **Current Upstream Limitations:** Based on the official model's training data, expression tags are currently most consistent in **English, Japanese, and Korean**. For other languages, the model may occasionally ignore them or read them out as plain text. Furthermore, some specific tags (like `<scream>` or `<angry>`) are known to be unstable across all languages.
> 
> 🔗 **Read more about this known issue from the community and developers in the [official Supertonic-3 discussion thread](https://huggingface.co/Supertone/supertonic-3/discussions/6).**

| Tag | Effect | Example Workaround Usage |
|-----|--------|--------------------------|
| `<breath>` | Soft inhale/exhale | `<breath> <breath> Hello, good morning everyone.` |
| `<laugh>` | Light chuckle/laugh | `Wow that's so funny! <laugh> <laugh> <laugh>` |
| `<sigh>` | Expressive sigh | `<sigh> <sigh> Oh well, it's okay.` |
| `<surprise>` | Surprised tone | `<surprise> <surprise> What? Did he really say that?` |
| `<scream>` | Yelling/Loud tone | `<scream> <scream> Watch out ahead!` |
| `<throatclear>` | Clearing throat | `<throatclear> <throatclear> Let's start the presentation.` |
| `<sad>` | Somber/Sad tone | `It's such a shame we lost today. <sad> <sad>` |
| `<angry>` | Aggressive/Angry | `<angry> <angry> Don't touch that!` |
| `<cough>` | Coughing sound | `<cough> <cough> Excuse me, just a moment.` |
| `<yawn>` | Yawning sound | `Whoa I'm so sleepy. <yawn> <yawn>` |

> 💡 **Pro Tip**: If a tag is ignored by the model, **repeat it 2-3 times** and place it at the absolute **beginning or end** of the sentence. Combining multiple tags also helps force the model to render the effect: `<laugh> <breath> it actually worked!`.
...
```bash
git clone [https://github.com/Anonymzx/Supertonic-WebUI.git](https://github.com/Anonymzx/Supertonic-WebUI.git)
cd Supertonic-WebUI
...
#   *Note: Depending on your hardware architecture (e.g., 1101), you may need to export HSA_OVERRIDE_GFX_VERSION=11.0.1 before launching.
...
| 🔊 Audio sounds robotic | Tags ignored by model | Use the workaround: repeat tags 2-3x at the start/end of the prompt |
